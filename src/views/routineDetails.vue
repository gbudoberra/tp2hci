  ...
  <v-container v-if="!loading">
    ...
    <template v-if="cycles.cycles">
      <v-row v-for="cycle in cycles.cycles.content" :key="cycle.id">
        <v-col>
          <workout-cycle :isMyRoutine="security.username===routines.routine.user.username"
                         :routineId="routines.routine.id"
                         :type="cycle.type" :order="cycle.order" :title="cycle.name" :detail="cycle.detail"
                         :repetitions="cycle.repetitions" :id="cycle.id"/>
        </v-col>
      </v-row>
      <v-row>
        <page-arrows v-on:nextPage="nextCyclePage" v-on:prevPage="prevCyclePage" :next-condition="!cycles.cyclesIsLast"
